        ...
        for i, (im_file, lb_file) in enumerate(pbar):
            try:
                ...
                if os.path.isfile(lb_file):
                    ...
                    if len(l):
                        assert l.shape[1] == 6, 'labels require 5 columns each, add angle'
...
def random_perspective(img, targets=(), segments=(), degrees=10, translate=.1, scale=.1, shear=10, perspective=0.0,
    ...
    if n:
        ...
        if use_segments:  # warp segments
            ...
            # clip
            new_ = new.copy()
            new_[:, [0, 2]] = new[:, [0, 2]].clip(0, width)
            new_[:, [1, 3]] = new[:, [1, 3]].clip(0, height)

        # filter candidates
        i = box_candidates(box1=targets[:, 1:5].T * s, box2=new_.T, area_thr=0.01 if use_segments else 0.10)
        targets = targets[i]
        targets[:, 1:5] = new[i]  # new
